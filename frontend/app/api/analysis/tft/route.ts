...
function scoreRSI(rsi: number): number {
  ...
  if (rsi < 65)  return -0.3;
  if (rsi < 75)  return -0.6;
  return -1.0;
}
...
function scoreBB(pos: number): number {
  if (pos < 0.1)  return  1.0;
  if (pos < 0.25) return  0.6;
  if (pos < 0.4)  return  0.2;
  if (pos < 0.6)  return  0.0;
  if (pos < 0.75) return -0.3;
  if (pos < 0.9)  return -0.7;
  return -1.0;
}

function scoreMACross(pct: number): number {
  return Math.max(-1, Math.min(1, pct / 5));
}

function scoreMomentum(ret: number): number {
  return Math.max(-1, Math.min(1, ret / 15));
}

function scoreVolatility(vol: number): number {
  if (vol < 20) return  0.3;
  if (vol < 35) return  0.0;
...
function toSignal(score: number): "strong_buy" | "buy" | "hold" | "sell" | "strong_sell" {
...
export async function GET(req: NextRequest) {
  ...
  const mom60    = calcMomentum(closes, 60);
  ...
  const factors: TftFactor[] = [
    {
      key:         "rsi",
      label:       "RSI (모멘텀)",
      category:    "past_dynamic",
      score:       scoreRSI(rsi),
      weight:      0.20,
      contribution: 0,
      value:       rsi.toFixed(1),
      description: rsi < 30 ? "과매도 — 반등 가능" : rsi > 70 ? "과매수 — 조정 가능" : "중립",
    },
    {
      key:         "macd",
      label:       "MACD (추세)",
      category:    "past_dynamic",
      score:       scoreMACD(macd.hist, macd.histPrev, price),
      weight:      0.18,
      contribution: 0,
      value:       `${macd.hist >= 0 ? "+" : ""}${macd.hist.toFixed(2)}`,
      description: macd.hist > 0 && macd.hist > macd.histPrev ? "상승 모멘텀 확대" : macd.hist < 0 ? "하락 추세" : "모멘텀 약화",
    },
    {
      key:         "bb",
      label:       "볼린저밴드 위치",
      ...
      description: bbPos < 0.2 ? "하단 밴드 — 과매도 구간" : bbPos > 0.8 ? "상단 밴드 — 과열 구간" : "밴드 중간 구간",
    },
    {
      key:         "volume",
      label:       "거래량 신호",
      category:    "past_dynamic",
      score:       Math.max(-1, Math.min(1, volSig)),
      weight:      0.15,
      contribution: 0,
      value:       `${volSig >= 0 ? "+" : ""}${(volSig * 100).toFixed(0)}%`,
      description: volSig > 0.3 ? "고거래량 상승 — 매수세 유입" : volSig < -0.3 ? "고거래량 하락 — 매도세" : "거래량 보통",
    },
    {
      key:         "ma_cross",
      label:       "이동평균 교차 (5/20)",
      category:    "past_dynamic",
      score:       scoreMACross(cross520),
      weight:      0.12,
      contribution: 0,
      value:       `${cross520 >= 0 ? "+" : ""}${cross520.toFixed(2)}%`,
      description: cross520 > 2 ? "골든크로스 상태" : cross520 < -2 ? "데드크로스 상태" : "교차 임박",
    },
    {
      key:         "mom20",
      label:       "20일 가격 모멘텀",
      category:    "past_dynamic",
      score:       scoreMomentum(mom20),
      weight:      0.12,
      contribution: 0,
      value:       `${mom20 >= 0 ? "+" : ""}${mom20.toFixed(1)}%`,
      description: mom20 > 10 ? "강한 상승 추세" : mom20 < -10 ? "강한 하락 추세" : "보합",
    },
    {
      key:         "volatility",
      label:       "연환산 변동성",
      category:    "static",
      score:       scoreVolatility(vol),
      weight:      0.08,
      contribution: 0,
      value:       `${vol.toFixed(1)}%`,
      description: vol < 20 ? "저변동성 — 안정적" : vol > 50 ? "고변동성 — 리스크↑" : "보통 변동성",
    },
  ];
