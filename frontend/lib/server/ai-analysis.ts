...
const SECTOR_TYPICAL_PER: Record<string, number> = {
  ...
  "통신":      11,
  "물류":      13,
  // universe.py 섹터 재정비로 신설된 키.
  // 지주: NAV 할인이 상시 반영돼 자회사 업종보다 낮은 PER이 정상.
  // 운송: 기존 해운(6)·물류(13) 통합 — 두 값의 중간대.
  "지주":      8,
  "운송":      9,
};
...
const SECTOR_ALIAS: Record<string, string> = {
  ...
  "물류":   "에너지",
  "유통":   "금융",
  // 신설 섹터: 운송(해운+물류 통합)은 기존 두 키와 동일한 프록시를 승계.
  // 지주는 전용 ETF가 없어 금융 ETF를 프록시로 사용.
  "운송":   "에너지",
  "지주":   "금융",
};
