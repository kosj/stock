...
export type SafeAssetType = "채권" | "현금성" | "금" | "채권혼합";
 ...
 *   - 금       : 금현물·골드 (전통적 안전자산. 단, 변동성은 채권보다 크다)
 *   - 채권혼합 : 주식이 섞인 혼합형. 제도상으로는 안전자산으로 인정되지만
 *                수익·손실이 주식 비중에서 크게 나오므로 순수 채권과 구분해
 *                표기한다(뭉뚱그리면 "안전"으로 오해될 수 있다).
 *
...
function classifySafeAsset(
  ...
  // 채권혼합: 주식이 섞인 혼합형 — 순수 채권보다 먼저 판정해야 한다
  if (/혼합/.test(name)) {
    return { safeAsset: true, safeType: "채권혼합" };
  }
  // 채권: 네이버 채권 탭 또는 명칭
