...
── sector 필드는 단순 표시용이 아니다 ────────────────────────────────────────
hybrid_ensemble.py 가 아래 두 곳에서 실제 신호로 사용한다:

  1) sector_rel_ret_5d / sector_rel_ret_20d
       = 종목 수익률 − 같은 날 동일 섹터 평균 수익률
     → 섹터에 이질적인 종목이 섞이면 "동종 대비 초과수익" 신호가 오염된다.
     → 섹터 구성원이 1종목이면 평균 = 자기 자신이라 값이 항상 0인 죽은 피처가
        된다. 어떤 섹터도 최소 2종목 이상을 유지할 것.

  2) SECTOR_CAP (Top-N 내 동일 섹터 상한)
     → 잘못된 라벨은 상한 슬롯을 엉뚱한 종목이 차지하게 만든다.

따라서 라벨 기준은 "사업 내용"이 아니라 "주가가 어느 집단과 함께 움직이는가"다.
지주회사는 자회사 사업과 무관하게 NAV 할인·배당에 연동되므로 별도 '지주'로 둔다.
"""

UNIVERSE = [
    # ── 반도체 (메모리·종합반도체) ────────────────────────────────────────────
    # 장비/소재는 '소부장', 지주사는 '지주'로 분리해 메모리 사이클 동조군만 남긴다.
    dict(ticker="005930", name="삼성전자",             sector="반도체",  market="KOSPI"),
    dict(ticker="000660", name="SK하이닉스",           sector="반도체",  market="KOSPI"),
    # 삼성전기: MLCC·FC-BGA 기판이 주력으로 반도체 수요 사이클과 동조 → 반도체 유지
    dict(ticker="009150", name="삼성전기",             sector="반도체",  market="KOSPI"),
    # ── 반도체 소재·부품·장비 ─────────────────────────────────────────────────
    dict(ticker="042700", name="한미반도체",           sector="소부장",  market="KOSDAQ"),
    dict(ticker="240810", name="원익IPS",              sector="소부장",  market="KOSDAQ"),
    dict(ticker="357780", name="솔브레인",             sector="소부장",  market="KOSDAQ"),
    dict(ticker="166090", name="하나머티리얼즈",       sector="소부장",  market="KOSDAQ"),
    # ── 지주회사 ──────────────────────────────────────────────────────────────
    # 자회사 업종이 아니라 NAV 할인·배당 정책에 연동되는 별도 집단.
    dict(ticker="003550", name="LG",                   sector="지주",    market="KOSPI"),
    dict(ticker="034730", name="SK",                   sector="지주",    market="KOSPI"),
    dict(ticker="036830", name="솔브레인홀딩스",       sector="지주",    market="KOSDAQ"),
    dict(ticker="267250", name="HD현대",               sector="지주",    market="KOSPI"),
    # ── 2차전지 ───────────────────────────────────────────────────────────────
    ...
    dict(ticker="336370", name="솔루스첨단소재",       sector="2차전지", market="KOSDAQ"),
    # ── 자동차 ────────────────────────────────────────────────────────────────
    ...
    dict(ticker="018260", name="삼성에스디에스",       sector="IT",      market="KOSPI"),
    # ── 게임 ──────────────────────────────────────────────────────────────────
    ...
    # 카카오뱅크: 인터넷전문은행 — 금리·대손 사이클에 연동되므로 IT가 아닌 금융
    dict(ticker="323410", name="카카오뱅크",           sector="금융",    market="KOSPI"),
    # ── 소재 ──────────────────────────────────────────────────────────────────
    ...
    # 씨에스윈드: 풍력 타워 제조 — 2차전지가 아니라 발전설비 투자 사이클에 연동
    dict(ticker="096530", name="씨에스윈드",           sector="유틸리티", market="KOSPI"),
    # ── 방산 ──────────────────────────────────────────────────────────────────
    ...
    # 한화오션(구 대우조선해양): 조선 3사와 수주 사이클을 공유 → 방산이 아닌 조선
    dict(ticker="042660", name="한화오션",             sector="조선",    market="KOSPI"),
    # ── 건설 ──────────────────────────────────────────────────────────────────
    ...
    # 한샘: 가구·인테리어 — 물류가 아니라 내구소비재(주택 거래량 연동)
    dict(ticker="004960", name="한샘",                 sector="소비재",  market="KOSPI"),
    # ── 통신 ──────────────────────────────────────────────────────────────────
    ...
    # ── 운송 (해운·항공·육상물류 통합) ────────────────────────────────────────
    # 기존 '해운'은 HMM 1종목뿐이라 sector_rel_ret 가 항상 0인 죽은 피처였다.
    # 운임·물동량이라는 공통 동인을 갖는 '물류'와 합쳐 3종목으로 만든다.
    dict(ticker="011200", name="HMM",                  sector="운송",    market="KOSPI"),
    dict(ticker="000120", name="CJ대한통운",           sector="운송",    market="KOSPI"),
    dict(ticker="003490", name="대한항공",             sector="운송",    market="KOSPI"),
]
